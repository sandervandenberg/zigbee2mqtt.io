...
### Pairing
Press the reset-button behind the battery cover for about 6 seconds. The light at the front should start blinking and it is possible to pair the device.
If pairing does not seem to work: press the button (the button itself, not the reset button) quickly three times in a row to change the mode the button is in. Then try pairing again.
